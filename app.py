# -*- coding: utf-8 -*-
"""
Created on Mon Sep  7 11:41:05 2020
...
def fn_create_chart3(Scon,Siso,Vminmaxyear,Vmonthsahead):
    ...
    return fig
#Not active!
@app.callback(Output('chart1', 'figure'),[Input('IsoSelection', 'value'),Input('PeriodSelection','value')])
def fn_create_chart1(Siso,Vminmaxyear):
    ...
    fig = plotlygraphs.Figure(chart_layout)
    fig.update_layout({"title": {"text": 'Average absolute forecast error ahead of release',"font": {"size": 14}}})
    
    #Key vectors and variables
    Vyears = [i for i in range(Vminmaxyear[0],Vminmaxyear[1]+1)]     #Define vector for years
    Vquartersahead = [12,8,6,4,2];Vx = [str(-nbq*3) + 'M' for nbq in Vquartersahead]
    Scon = 'gdpr';
    ...
    fig.add_trace(plotlygraphs.Bar(x=Vx,y=Vy,name=' Trace 1'))
        ...
        for Sprovider in Vproviders:
            ...
            if Smode == 'Lines':
                ...
                if Bcumul == True:
                    ...
                    if Btunnel == False or (Sprovider!='low' and Sprovider!='high' and Sprovider !='std'):
